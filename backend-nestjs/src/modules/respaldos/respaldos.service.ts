...
export class RespaldosService {
  ...
  private normalizeColumnValue(value: any, dataType: string) {
    ...
    if (!['date', 'datetime', 'timestamp'].includes(dataType)) return value;
    ...
    try {
      ...
      const detalles = await Promise.all(
        tables.map(async (table) => {
          const [row] = await this.dataSource.query(`SELECT COUNT(*) AS count FROM ${this.quoteIdentifier(table)}`);
  ...
  async exportar(req: any, res: any) {
    try {
      ...
      for (const table of tables) {
        const rows = await this.dataSource.query(`SELECT * FROM ${this.quoteIdentifier(table)}`);
  ...
  async restaurar(body: any, req: any, res: any) {
    ...
    try {
      ...
      for (const table of existingTables.slice().reverse()) {
        await queryRunner.query(`DELETE FROM ${this.quoteIdentifier(table)}`);
      }
      ...
      for (const table of backupTables) {
        ...
        for (const row of rows) {
          ...
          const sql = `INSERT INTO ${this.quoteIdentifier(table.name)} (${columns.map((column) => this.quoteIdentifier(column)).join(', ')}) VALUES (${placeholders})`;
